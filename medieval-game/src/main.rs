...
//   appropriate (it's a Generic variable, so it'll adapt to load any file).
// The "windows" variable of type 'ResMut<Windows>' is a resource (ResMut<_>) that will
//   allow you to access and alter the window's components (properties).
fn setup_system(
    ...
    // Spawn Main sprite entity -- using SpriteBundle (sprite builder)
    commands.spawn_bundle(SpriteBundle {
        // uses asset_server to load image and returns a handle (pointer?)
        texture: asset_server.oad(PLAYER_SPRITE),
        ..Default::default() // add default properties for the rest
    });
}
